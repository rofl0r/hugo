     ...
     ARCH_FILE fichier[1600], char *must_draw, TO_DISPLAY to_display[1600])
{
  ...
  #define init_research(tmp_dir) (findfirst (tmp_dir, &DirBuf, FA_DIREC|FA_RDONLY|FA_ARCH) == 0)
  #define continue_research (findnext (&DirBuf) == 0)
  #define found_attrib (DirBuf.ff_attrib)
  #define found_name (DirBuf.ff_name)
  #define close_research
#elif defined(WIN32)
  struct _finddata_t finddata;
  int research_handle;
  #define init_research(mask) ((research_handle = _findfirst(mask, &finddata)) != 1)
  #define continue_research (_findnext(research_handle, &finddata) == 0)
  #define found_attrib (finddata.attrib)
  #define found_name (finddata.name)
  #define close_research _findclose(research_handle)
#endif
  unsigned i, dummy;
  ...
  *must_draw = 1;
  ...
  if (init_research(tmp_dir))      // Look for the first entry
    do
      {
        if (((found_attrib & 0x10) != 0)
            && (strcmp (found_name, ".") != 0))
          {
            strcpy (direct[(*dirt)++], strupr (found_name));
            direct[(*dirt) - 1][13] = 0;
          }
      }
    while (continue_research);    // until there's no more
  close_research;
...
#define add_files_arch(mask) \
      strcpy (tmp_dir, nom_dir); \
      strcat (tmp_dir, mask); \
      if (init_research(tmp_dir)) \
        do \
          { \
            if ((found_attrib & 0x10) == 0) \
              { \
                (*arch)++; \
                strcpy (fichier[i].basefilename, strupr (found_name)); \
                strcpy (fichier[i].archived, (found_name)); \
                strcpy (to_display[i].str, (found_name)); \
                to_display[i].corr = i; \
                i++; \
              } \
          } \
        ...
        /* Adding files not included in archives */
        add_files_arch("/*.BIN");
        add_files_arch("/*.ISO");
        add_files_arch("/*.ISQ");
        add_files_arch("/*.HCD");
      ...
      if (init_research(tmp_dir)) \
        do \
          { \
            if ((found_attrib & 0x10) == 0) \
              { \
                strcpy (fi[(*fich)++], strupr (found_name)); \
                fi[(*fich) - 1][13] = 0; \
              } \
          } \
        while (continue_research); \
              ...
              if ((found_attrib & 0x10) == 0)
                // We have found an archive, let's get in!
                {
                  ...
                  sprintf (tmp_char, "%sHU-GO!.TMP/REDIR.TMP",
                           short_exe_name);
                  sprintf (tmp_char2, "%s/%s", nom_dir, found_name);
                  ...
                  if (redir_file = fopen (tmp_dir_arch, "rt"))
                    {
                      ...
                      while (fgets (tmp_buf, 0x100, redir_file))
                        if (strcasestr (tmp_buf, ".PCE"))
                          {
                            strcpy (base_arch[*arch], found_name);
                            base_arch[*arch][13] = 0;
            ...
            if (strstr (fi[dummy], ".PCE") != NULL)
              ...
              for (x = 0; x < nb_item_in_index; x++)
                if (!strcmp (index[x].basefilename, fi[dummy]))
                  {
                    Log("Adding %s in the list", index[x].romname);
