...
UInt16 read_joypad (int port) 
{
  ...
  if (joypad[port].axis[1] > 0)
    tmp |= JOY_DOWN;
  if (joypad[port].
	button[config[current_config].
	       joy_mapping[port][J_PSTART]] 
	||key[config[current_config].joy_mapping[port][J_START]])
    tmp |= JOY_START;
  if (joypad[port].
	button[config[current_config].
	       joy_mapping[port][J_PSELECT]] 
...
if (key[KEY_ASTERISK])
{
      UInt32 sav_timer = timer_60;      if (sound_driver == 1)	osd_snd_set_volume (0);
      disass_menu ();
      if (sound_driver == 1)	osd_snd_set_volume (gen_vol);
      timer_60 = sav_timer;      key_delay = 10;
	...
	
	{
	  if (dump_snd)
	    ...
	    
	    {
	      ...
	      while ((tmp < 0xFFFF) && ((out_snd=fopen(tmp_buf,"rb")) != NULL))
		  {
			  fclose(out_snd);
	...
	switch (event.type)
	{
		case SDL_KEYDOWN:
			switch (event.key.keysym.scancode)
			{
				case SDLK_ASTERISK:
    {      UInt32 sav_timer = timer_60;
      if (sound_driver == 1)	osd_snd_set_volume (0);
      disass_menu ();
      if (sound_driver == 1)	osd_snd_set_volume (gen_vol);
      timer_60 = sav_timer;
      key_delay = 10;
break;    }
  	...
  	case SDLK_F4:    
    {
      UInt32 sav_timer = timer_60;      UChar error_code;
      // error_code = gui ();
      key_delay = 10;      timer_60 = sav_timer;
...
case SDLK_1:
	...
	{	  SPONSwitch = !SPONSwitch;	  key_delay = 10;	
		break;}
      

case SDLK_2:
	{	  	BGONSwitch = !BGONSwitch;	 	key_delay = 10;	
		break;	}    
  case SDLK_F1:
    {      UInt32 sav_timer = timer_60;   if (sound_driver == 1)	osd_snd_set_volume (0);
      searchbyte ();      if (sound_driver == 1)	osd_snd_set_volume (gen_vol);      timer_60 = sav_timer;      return 0;    }
...
case SDLK_4:    _Wr6502 (0x2066, Rd6502 (0x2066) | 32);
break;
  case SDLK_5:    cd_port_1800 = 0xD8;
break;
  case SDLK_6:    _Wr6502 (0x22D6, 1);
break;
  
case SDLK_8:    _Wr6502 (0x20D8, 128);
break;          
...
break;}
  case SDLK_0:
    {
      if (freq_int < 44100)	freq_int += 50;
if (sound_driver == 1)
	
	{
	  #warning need to implement downmixing// voice_set_frequency (PCM_stream->voice, freq_int);
	  sprintf (tmp_buf, MESSAGE[language][freq_set_to], freq_int);
	  osd_gfx_set_message (tmp_buf);
	  message_delay = 60;
	}
  ...
  case SDLK_9:    
	...
	{
	 #warning need to implement upmixing //voice_set_frequency (PCM_stream->voice, freq_int);
	  sprintf (tmp_buf, MESSAGE[language][freq_set_to], freq_int);
	  osd_gfx_set_message (tmp_buf);
	  message_delay = 60;
	}
