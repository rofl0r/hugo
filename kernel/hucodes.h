...
#define MC_Id(Rg)	K.W=MCZp(); Rg.B.l=RdRAM(K.W);Rg.B.h=RdRAM(K.W+1)
#define MR_Id(Rg)	MC_Id(J);Rg=Rd6502(J.W)
//#define	M_FL2(Rg)	_P&=~(Z_FLAG|N_FLAG|V_FLAG);_P|=(Rg&0xc0)|ZNTable[Rg]
#define	TSB(Rg)	_NF=_VF=Rg; Rg|=_A; _ZF=Rg;
#define	TRB(Rg) _NF=_VF=Rg; Rg&=~_A; _ZF=Rg;
#define T_INIT() word src,dist,length; src  = Op6502w(_PC_); _PC_+=2; dist = Op6502w(_PC_); _PC_+=2; length = Op6502w(_PC_); _PC_+=2; cycle=length*6
#define M_ADCx(Rg) { byte *Mx=AdrRAM(_X+ZP);if(_P&D_FLAG) { K.B.l=(*Mx&0x0F)+(Rg&0x0F)+(_P&C_FLAG); K.B.h=(*Mx>>4)+(Rg>>4);/*+(K.B.l>>4);*/ if(K.B.l>9) { K.B.l+=6;K.B.h++; } if(K.B.h>9) K.B.h+=6; *Mx=(K.B.l&0x0F)|(K.B.h<<4);_P=(_P&~C_FLAG)|(K.B.h>15? C_FLAG:0);_ZF=_NF=*Mx;cycle++; } else { K.W=*Mx+Rg+(_P&C_FLAG);_P&=~C_FLAG; _P|=(K.B.h? C_FLAG:0); _VF=(~(*Mx^Rg)&(*Mx^K.B.l))>>1;_ZF=_NF=K.B.l; *Mx=K.B.l; } }
