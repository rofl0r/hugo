...
/* BRK */
case 0x00:
  fprintf(stderr,"BRK instruction at IP=%04x\nCurrent CD command was : %x\n",_PC_-1,Rd6502(0x224c));
...
//Zeo
//case 0xA5: R.A = RAM[ (Op6502(R.PC.W++)) ]  ;break;
//CAUTION ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
default:
  if(_TrapBadOps) {
    ...
    ( stderr,
      ...
      (unsigned long)_User,Op6502(_PC_-1),(word)(_PC_-1)
    );
