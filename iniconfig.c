...
int config_var_cmp(const void *lhs, const void *rhs)
{
...
const char *joy_str[] =
{
  "UP", "DOWN", "LEFT", "RIGHT", "I", "II", "SELECT", "START", "AUTOI", "AUTOII",
  "PI", "PII", "PSELECT", "PSTART", "PAUTOI", "PAUTOII", "PXAXIS", "PYAXIS"
};


void read_joy_mapping(void)
{
  ...
  {  
    if (z < 9)
      section_name[6] = '0' + (z + 1);
    else
      section_name[6] = z - 9 + 'a';
    ...
    { // for each joystick
      ...
      /* Zero out our gamepad mappings--needed so we can tell whether or not they should be configured */
      for (y = J_PAD_START; y < J_MAX; y++)
        config[z].joy_mapping[x][y] = 0;
		
      strcpy(tmp_str2, "input1");
      tmp_str2[5] = '0' + (x + 1);
		
      config[z].input_type[x] = NONE;

      strncpy(tmp_str, get_config_string(section_name, tmp_str2, "none"), 10);
      ...
      if (stricmp(tmp_str, "none"))
      {
        ...
        if (!stricmp (tmp_str, "key1"))
          config[z].input_type[x] = KEYBOARD1;
        else if (!stricmp (tmp_str, "key2"))
          config[z].input_type[x] = KEYBOARD2;
        else if (!stricmp (tmp_str, "key3"))
          config[z].input_type[x] = KEYBOARD3;
        else if (!stricmp (tmp_str, "key4"))
          config[z].input_type[x] = KEYBOARD4;
        else if (!stricmp (tmp_str, "key5"))
          config[z].input_type[x] = KEYBOARD5;
        else if (!stricmp (tmp_str, "joy1"))
          config[z].input_type[x] = JOYPAD1;
        else if (!stricmp (tmp_str, "joy2"))
          config[z].input_type[x] = JOYPAD2;
        else if (!stricmp (tmp_str, "joy3"))
          config[z].input_type[x] = JOYPAD3;
        else if (!stricmp (tmp_str, "joy4"))
          config[z].input_type[x] = JOYPAD4;
        else if (!stricmp (tmp_str, "mouse1"))
          config[z].input_type[x] = MOUSE1;
        else if (!stricmp (tmp_str, "mouse2"))
          config[z].input_type[x] = MOUSE2;
        else if (!stricmp (tmp_str, "synaplink"))
          config[z].input_type[x] = SYNAPLINK;

        for (y = 0; y < J_MAX; y++)
        {
          strncpy(tmp_str, joy_str[y], 10);
          tmp_str[strlen (tmp_str) + 1] = 0;
          tmp_str[strlen (tmp_str)] = (x + 1) + 0x30;

          if ((temp_val = get_config_int(section_name, tmp_str, 0xffff)) != 0xffff)
          {
            config[z].joy_mapping[x][y] = temp_val;
            Log("    %s set to %d\n", joy_str[y], temp_val);
          }
        }
      }
  ...
  switch (nb_arg)
    ...
    case 'm':
      ...
      return 0;
		...
		case 'n':
			netplay_mode = atoi(val);
		  Log("Netplay mode set to %d\n", netplay_mode);
		  return 0;
		case 'N':
			netplay_mode = 2; // client mode
			server_hostname = strdup(val);
		  // strncpy(server_hostname, val, 256);
		  Log("Netplay mode set to %d\nServer hostname set to %s\n", netplay_mode, server_hostname);
			return 0;
#endif // NETPLAY
    case 's':
...
parse_INIfile_raw ()
{
  ...
  freq_int = get_config_int ("main", "snd_freq", 22050);
  ...
  option.want_fullscreen_aspect = get_config_int("main", "use_fullscreen_aspect", 0);
  ...
  option.fullscreen_height = get_config_int("main", "fullscreen_height", 480);
  ...
  Log ("Minimum Bios hooking set to %d\n", minimum_bios_hooking);
	...
	#warning with gcc2.95.4 20011002 under linux, the following statement crashes. It is certainly a gcc bug	
...
void
set_config_var_str(char* section, const char* name, char* value)
{
    int index = 0;
	config_var key, *result;
...
void
set_config_var_int(char* section, const char* name, int value)
{
	char temp_string[10];
	...
	for (local_index = 0; local_index < config_ar_index; local_index ++)
		{
			...
			fprintf(output_file, "%s=%s\n", config_ar[local_index].variable, config_ar[local_index].value);
		}
...
save_config()
{
	...
	char input_config_number, input_config_button, input_config_player;
	...
	set_config_var_int("main","snd_freq", freq_int);
	...
	set_config_var_int("main","use_fullscreen_aspect", option.want_fullscreen_aspect);
	...
	for (input_config_number = 0; input_config_number < 16; input_config_number++)
		{
			...
			for (input_config_player = 0; input_config_player < 5; input_config_player++)
				{
					char input_name[7];
					char input_type_name[10];
					
					snprintf(input_name, 7, "input%1d", input_config_player + 1);
					
					switch (config[input_config_number].input_type[input_config_player])
						{
							case KEYBOARD1:
								strcpy(input_type_name, "key1");
							  break;
							case KEYBOARD2:
								strcpy(input_type_name, "key2");
							  break;
							case KEYBOARD3:
								strcpy(input_type_name, "key3");
							  break;
							case KEYBOARD4:
								strcpy(input_type_name, "key4");
							  break;
							case KEYBOARD5:
								strcpy(input_type_name, "key5");
							  break;
							case JOYPAD1:
								strcpy(input_type_name, "joy1");
							  break;
							case JOYPAD2:
								strcpy(input_type_name, "joy2");
							  break;
							case JOYPAD3:
								strcpy(input_type_name, "joy3");
							  break;
							case JOYPAD4:
								strcpy(input_type_name, "joy4");
							  break;
							case MOUSE1:
								strcpy(input_type_name, "mouse1");
							  break;
							case MOUSE2:
								strcpy(input_type_name, "mouse2");
							  break;
							case SYNAPLINK:
								strcpy(input_type_name, "synaplink");
							  break;							
							default:
							  strcpy(input_type_name, "none");
						}
						
						// Set the device used for this player
						set_config_var_str(section_name, input_name, input_type_name);

						for (input_config_button = 0; input_config_button < J_MAX; input_config_button ++)
						  {
								char temp_joy_str[15];
							 
							    // Skip empty entries in joypad mapping
							    if (config[input_config_number].joy_mapping[input_config_player][input_config_button] == 0)
									continue;
							  
								snprintf(temp_joy_str, 15, "%s%1d", joy_str[input_config_button], input_config_player);
							    set_config_var_int(section_name, temp_joy_str, config[input_config_number].joy_mapping[input_config_player][input_config_button]);
						  }
				}
		}
