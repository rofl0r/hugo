 ...
 switch (imm_operand(reg_pc+1))
  {
   ...
   case CD_READ:
        ...
        case CD_STAT:
              ...
              */
             reg_p = ((reg_p & (~(FL_N|FL_T|FL_Z))) | flnz_list[reg_a = (timer_60&64?0x80:0x00)]);
             rts();
             break;
        ...
        case AD_TRANS:
             {
              ...
              pce_cd_sectoraddy += (get_8bit_zp(0x2274+3*get_8bit_zp(0x2273)) << 16 )+
                                   (get_8bit_zp(0x2275+3*get_8bit_zp(0x2273)) << 8 )+
                                   (get_8bit_zp(0x2276+3*get_8bit_zp(0x2273)));
        ...
        case AD_PLAY:
             ...
             io.adpcm_rate = 32 / (16 - (get_8bit_zp(_dh) & 15));
        ...
        case AD_STAT:
             /* TODO : really return info concerning the current state of
              * ADPCM playing
              */
             {
              static ret_val = 0;
              ...
              reg_p = ((reg_p & (~(FL_N|FL_T|FL_Z))) | flnz_list[reg_a = ret_val]);
             }
             rts();
             break;
...
*/
			...
			put_8bit_addr(buf_offset  , binbcd[Min]);
			put_8bit_addr(buf_offset+1, binbcd[Sec]);
			put_8bit_addr(buf_offset+2, binbcd[Fra]);
			put_8bit_addr(buf_offset+3, Ctrl);
                ...
                switch (CD_emulation)
                  {
                   ...
                   case 3:
                   case 4:
                        put_8bit_addr(buf_offset  ,binbcd[01]); // Number of first track  (BCD)
                        put_8bit_addr(buf_offset+1,binbcd[nb_max_track]); // Number of last track (BCD)
                        break;
                   case 1:
		      {
...
/*
                        Log("Min track = %d\nMax track = %d\n",
                            ...
                            last_track);
*/
			put_8bit_addr(buf_offset  ,binbcd[first_track]);
			put_8bit_addr(buf_offset+1,binbcd[last_track]);
...
*/
                        put_8bit_addr(buf_offset  ,binbcd[HCD_first_track]);
                        put_8bit_addr(buf_offset+1,binbcd[HCD_last_track]);
                        break;
                  }
                }
                reg_p = ((reg_p & (~(FL_N|FL_T|FL_Z))) | flnz_list[reg_a = 0]);
              ...
              case CD_DINFO_LENGTH:
                ...
                Log("bios.c: Sub function CD_LENGTH from CD_DINFO bios function isn't hard handled yet\n");
...
*/
              if (get_8bit_zp(_bh)==0x80)
              ...
              else
                {
                 ...
                 int min1 = bcdbin[get_8bit_addr(_al)];
                 int sec1 = bcdbin[get_8bit_addr(_ah)];
                 int fra1 = bcdbin[get_8bit_addr(_bl)];

                 int min2 = bcdbin[get_8bit_addr(_cl)];
                 int sec2 = bcdbin[get_8bit_addr(_ch)];
                 int fra2 = bcdbin[get_8bit_addr(_dl)];
                ...
                for (index=0;index<5;index++)
                   {
                    ...
                    put_8bit_addr(0x2232+index,dummy[index]);
                    put_8bit_addr(0x2228+index,io.JOY[index]);
                    put_8bit_addr(0x222D+index,(io.JOY[index]^dummy[index])&io.JOY[index]);
                    }
	...
	case BM_FREE:
             {
              ...
              put_8bit_zp(_cl, free_mem & 0xff);
              put_8bit_zp(_ch, free_mem >> 8);
